...
int main(int argc, char** argv) {
    ...
    // Manually sum up all partial sums at the root
    if (rank == 0) {
        total_sum = partial_sum;  // Start with the root's partial sum
        for (int i = 1; i < size; i++) {
            int recv_partial_sum;
            MPI_Recv(&recv_partial_sum, 1, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            total_sum += recv_partial_sum;
        }
        printf("Total sum of array elements is %d\n", total_sum);
    } else {
        // Non-root processes send their partial sum to the root
